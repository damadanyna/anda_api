...
class Employe {
    ...
    static async findList(req, res) {
        ...
        filters.sort_by = (!filters.sort_by) ? _obj_pat[default_sort_by] : _obj_pat[filters.sort_by]
        try {
            let reponse = await D.exec_params(`select employe.* 
             from  employe where 
             employe.emp_nom_prenom like ?  order by ${'employe.'+filters.sort_by} limit ? offset ? `, [
                    '%' + req.body.find + '%',
                    filters.limit,
                    (filters.page - 1) * filters.limit
                ])
                //Liste total des conge
            let nb_total_conge = (await D.exec('select count(*) as nb from employe'))[0].nb
